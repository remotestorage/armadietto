...
const { substituteInCandidateResponse, writeOutRealResponse } = require('./utils/responseCandidate');
...
class Armadietto {
  ...
  buildMiddleware () {
    if (!this._options.middleware || this._options.middleware.length === 0) return;
    // middleware stack called in order defined:
    for (const Middleware of this._options.middleware) {
      if (Middleware.isEnabled(this._options)) {
        this._middleware.push((req, res, next) => new Middleware(this, req, res, next, this._options).handle);
      }
    }
  }
  ...
  handle (req, res) {
    ...
    req.on('end', async () => {
      ...
      try {
        // handler stack, top down:
        const middleware = async () => await this.middleware(req, res, this.dispatch);
        await this.preface(req, res, middleware);
      } catch (e) {
        logRequest(req, '-', 500, '-', 'DISPATCH ERROR: ' + e.stack);
      }
    });
  ...
  async preface (req, res, next) {
    // eslint-disable-next-line no-unused-vars
    const [_, uri, isBasePathMatch] = getRouting(req, this._options);
  ...
  async middleware (req, res, last) {
    const candidateResponse = substituteInCandidateResponse(res);

    let next = async () => await last.bind(this)(req, candidateResponse);
    this._middleware.slice().reverse().forEach(m => {
      next = m(req, candidateResponse, next);
    });
    await next();

    writeOutRealResponse(candidateResponse);
  }

  async dispatch (req, res) {
    const [method, uri] = getRouting(req, this._options);
