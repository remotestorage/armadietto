...
document.querySelector('table')?.addEventListener('click', resendInvite);
...
async function submit(data) {
  try {
    ...
    if (resp.ok) {
      ...
      displayOutput(invite.text + '\n' + invite.url, 'Copy and paste this invite to a secure channel:');
    } else {
  ...
  try {
    ...
    switch(contactURL.protocol) {
      ...
      case 'xmpp:': // xmpp:username@domain.tld
        ...
        break;
      case 'skype:': // skype:<username|number>?[add|call|chat|sendfile|userinfo]
        contactURL.search = `?chat`;
        mode = 'from my Skype account';
        break;
      case 'msnim:': // msnim:[add|chat|voice|video]?<email>
        mode = 'from my Windows Live Messenger account';
        break;
      case 'mailto:':
