...
async function getOptions() {
  try {
    ...
    if (response.ok) {
      ...
      displayMessage('Click the button below to create a passkey for this device');
    } else {
      const body = await response.json();
      if (body.error) {
        displayMessage(body.error, true);
      } else {
        displayMessage(`Something went wrong. Request another invite using the link above.`, true);
      }
    }
  } catch (err) {
...
async function generateCatchingErrors() {
  try {
    ...
    if (err.code === "ERROR_AUTHENTICATOR_PREVIOUSLY_REGISTERED") {
      ...
      displayMessage('You have already created a passkey for this device. Just log in, using the “Log in” link above!');
      await cancelInvite()
...
async function generateAndRegisterPasskey() {
  ...
  if (verificationResp.ok) {
    ...
    let msg = 'Check your connection';
    try {
      const failJSON = await verificationResp.json();
      msg = failJSON.error || msg;
    } catch (err) {
      console.error(`while parsing fail JSON:`, err);
    }
