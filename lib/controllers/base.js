...
class Controller {
  ...
  renderXRD (file, locals) {
    ...
    const template = this.readFile(viewDir + file)
  ...
  renderJSON (data, contentType) {
    var body = Buffer.from(JSON.stringify(data, true, 2))
  ...
  renderHTML (status, file, locals) {
    ...
    const layout = this.readFile(path.join(viewDir, '/layout.html'))
    console.error('sono dentro renderHTML ', status)
    console.error('file', layout)
    const body = this.readFile(path.join(viewDir, file))
    ...
    var html = Buffer.from(ejs.render(layout.toString(), globals))
