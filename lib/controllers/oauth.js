...
class OAuth extends Controller {
  ...
  async authenticate () {
    ...
    const [error] = await this.server._store.authenticate({username: username, password: params.password});
    if (error) {
      ...
      const [error, token] = await this.server._store.authorize(params.client_id, username, permissions);//, (error, token) => {
      const args = {
        access_token: token,
        token_type: 'bearer'
      };
