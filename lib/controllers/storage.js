...
class Storage extends Controller {
  constructor (server, request, response, username, path) {
    ...
    this._path = path;
    console.error('sono nel costruttore di storage', username);
  ...
  async get () {
    ...
    console.error('sono proprio nella get');
    if (await this.checkToken('r')) {
      try {
        var {item, error, versionMatch} = await this.server._store.get(this._username, this._path, version);
      } catch (e) {
        console.error('Your storage backend does not behave correctly');
      }
  ...
  async put () {
    ...
    if (await this.checkToken('w')) {
      try {
        var { created, modified, conflict } = this.server._store.put(this._username, this._path, type, value, version);
        status = conflict ? 412
          : created ? 201
            : 200;
      } catch (e) {
