...
class Storage extends Controller {
  ...
  async get () {
    ...
    if (await this.checkToken('r')) {
      try {
        ...
        this.response.writeHead(500, e.message);
  ...
  async delete () {
    ...
    const {deleted, modified, conflict} = this.server._store.delete(this._username, this._path, version);
    const status = deleted ? 200
      : conflict ? 412
        : 404;
