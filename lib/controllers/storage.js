...
class Storage extends Controller {
  ...
  async get () {
    ...
    if (await this.checkToken('r')) {
      ...
      if (item && item.items) {
        this._headers['Content-Type'] = 'application/ld+json';
        item.value = Buffer.from(JSON.stringify({
  ...
  async delete () {
    ...
    try {
      var {deleted, modified, conflict} = await this.server._store.delete(this._username, this._path, version);
      status = deleted ? 200
        : conflict ? 412
          : 404;
    } catch (e) {
