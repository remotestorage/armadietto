...
class Storage extends Controller {
  ...
  async get (head = false) {
    ...
    if (await this.checkToken('r')) {
      try {
        ...
        this.server._logger.error(`Your storage backend does not behave correctly => ${e.message}`);
  ...
  async checkToken (permission) {
    ...
    try {
      ...
      this.server._logger('Bad store.permissions implementation?', { error: e.message });
