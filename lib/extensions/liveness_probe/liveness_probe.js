...
class LivenessProbe extends Controller {
   ...
   * @returns {bool} whether this class is enabled if `options` indicate as much
   */
  static isEnabled (options) {
    return options.extensions?.liveness_probe?.enabled;
  }
    ...
    try {
      ...
      if (!user) {
        await this.server._store.createUser({
          ...
          email: `liveness_probe@${core.generateToken()}.fake`,
