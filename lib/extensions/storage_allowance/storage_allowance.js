...
const { metadataUpsert, metadataDelete, getMetadata } = require('../utils/token_metadata');
...
class StorageAllowance extends Controller {
  ...
  static isEnabled (options) {
    return options.extensions?.storage_allowance?.enabled;
  }
  ...
  handle = async () => {
    ...
    if (method === 'POST' && (uri.pathname === 'oauth' || uri.pathname === 'pay2myapp')) {
      ...
      let value = 'false';
      if (hasCapacity) {
        value = symmetricEncrypt(`${HAS_WRITE_CAPACITY_METADATA_VALUE_PREFIX}::${(new Date()).toUTCString()}`, salt);
      }
