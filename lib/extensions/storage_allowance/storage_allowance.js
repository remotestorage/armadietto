...
class StorageAllowance extends Controller {
  static client = null;
  static consumptionLimiterRedis = null;
  ...
  getAvailable (username) {
    ...
    try {
      if (!fs.existsSync(datapath)) {
        return [0, this._options.extensions.storage_allowance.max_bytes];
      }
    } catch (err) {
      return [0, this._options.extensions.storage_allowance.max_bytes];
    }
  ...
  async updateCapacity (sizeDelta, tokenMetadata, authToken) {
    try {
      ...
      const currentTracked = await StorageAllowance.consumptionLimiterRedis.get(key);
      let consumed = 0;
      if (!currentTracked) {
      ...
      await StorageAllowance.consumptionLimiterRedis.set(key, newValue, 0);
  ...
  static connect = async (options) => {
    ...
    StorageAllowance.consumptionLimiterRedis = new RateLimiterRedis({
