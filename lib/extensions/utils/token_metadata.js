...
function metadataDelete(token, key) {
  ...
  let metadata = JSON.parse(Buffer.from(matched[1],'base64').toString());    
  delete metadata[key];
  const strippedToken = token.replace(METADATA_REGEX, '');
...
module.exports = { metadataUpsert: metadataUpsert, metadataDelete: metadataDelete, getMetadata: getMetadata }