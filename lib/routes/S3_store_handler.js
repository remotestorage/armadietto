...
const PUT_TIMEOUT = 1 * 60 * 60 * 1000;
const MAX_KEY_LENGTH = 942; // MinIO in /var/minio; OpenIO: 1023; AWS & play.min.io: 1024
...
module.exports = function (endPoint = 'play.min.io', accessKey = 'Q3AM3UQ867SPQQA43P2F', secretKey = 'zuf+tfteSlswRu7BJ86wekitnifILbZam1KYY3TG', region = 'us-east-1', userNameSuffix = '') {
  ...
  router.put('/:username/*',
    async function (req, res, next) {
      try {
        ...
        const putETag = normalizeETag(await putBlob(bucketName, s3Path, contentType, contentLength, req));
  ...
  async function updateAncestors (bucketName, s3Path, putETag, documentMetadata) {
    ...
    do {
      ...
      const [documentETag] = await head(bucketName, s3Path);
      if (documentETag !== putETag) {
