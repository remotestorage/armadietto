    ...
    async function (req, res, next) {
      ...
      try {
        ...
        try {
          ...
          } else if (['NotFound', 'NoSuchKey', '404'].includes(err.name) ||
      ...
      try {
        ...
        do {
          ...
          for (const entry of Contents || []) {
            let match;
            if (entry.Key === s3Path) {
              currentETag = stripQuotes(entry.ETag);
        ...
        } else if (req.get('If-None-Match') && stripQuotes(req.get('If-None-Match')) === currentETag) {
          res.logNotes.add(`version ${currentETag} is already here`);
          res.logLevel = 'info';
          res.status(412).end(); return;
        } else if (req.get('If-Match') && stripQuotes(req.get('If-Match')) !== currentETag) {
    ...
    async function (req, res, next) {
      ...
      try {
        ...
        if (keys?.[0]?.Key === s3Path) { // document exists
          ...
          currentETag = keys?.[0]?.ETag;
          if (req.get('If-Match') && stripQuotes(req.get('If-Match')) !== stripQuotes(currentETag)) {
            res.logNotes.add(`If-Match:${req.get('If-Match')} actually ${currentETag}`);
            return res.status(412).end();
          } else if (req.get('If-None-Match') && stripQuotes(req.get('If-None-Match')) === stripQuotes(currentETag)) {
            res.logNotes.add(`If-None-Match:${req.get('If-Match')} actually ${currentETag}`);
            return res.status(412).end();
          }
      ...
      if (['Enabled', 'Suspended'].includes(versioningResult?.Status)) {
        ...
        do {
          ...
          if (Versions?.length > 0) {
            Objects.push(...Versions.map(version => ({ Key: version.Key, VersionId: version.VersionId })));
          }
          if (DeleteMarkers?.length > 0) {
            Objects.push(...DeleteMarkers.map(delMarker => ({ Key: delMarker.Key, VersionId: delMarker.VersionId })));
          }
          if (Objects?.length > 0) {
            const { Deleted, Errors } = await s3send(new DeleteObjectsCommand({ Bucket: bucketName, Delete: { Objects } }));
...
function respondWithMsgId (res, statusCode = 500, err, userMsg) {
  ...
  errToMessages(err, res.logNotes);
  if (err.$responseBodyText) { res.logNotes.add(err.$responseBodyText); }
...
function stripQuotes (ETag) {
  if ('string' !== typeof ETag) { return ETag; }
  return ETag.replace(/^"W\/"|^"|^W\/"|"$/g, '');
}
