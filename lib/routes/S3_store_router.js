...
// const CONTENT_TYPE_SEPARATOR = '\u2610'; // ballot box; separates Content-Type from path
const TYPE_SUFFIX_PATT = /\u2610(application|audio|font|image|model|text|video)\/[a-z0-9][a-z0-9+.!#$&^_-]{1,63}$/i;
...
const BUSY_RETRY_AFTER = Math.round(INITIAL_S3_PAUSE_MS / 1000).toString();
...
module.exports = function ({ endPoint = 'play.min.io', accessKey = 'Q3AM3UQ867SPQQA43P2F', secretKey = 'zuf+tfteSlswRu7BJ86wekitnifILbZam1KYY3TG', region = 'us-east-1', userNameSuffix = '' }) {
  ...
  const MAX_WAITING_S3_REQUESTS = 10; // client put might require 10 requests to S3
      ...
      try {
        ...
        try {
          if (['NoSuchBucket', 'AccessDenied'].includes(err.Code) ||
            ...
            if (isFolderRequest) {
              const folder = await listFolder(req.params.username, req.params[0], res.logNotes);
              ...
              if (Contents?.length > 0) {
                ...
                return res.status(409).type('text/plain').send('is actually folder: ' + s3Path.slice(BLOB_PREFIX.length));
              } else {
                errToMessages(err, res.logNotes);
                return res.status(404).end(); // Not Found
              }
   ...
   * @param username
   * @param {string} targetPath the prefix of the folder
   * @param {Set} logNotes
   * @return {Promise<Object>} metadata of the items: relative path, contentLength, ETag, lastModified
   */
  async function listFolder (username, targetPath, logNotes) {
    ...
    const existingFolders = new Set(); // paths of existing cached folders

    const headQueue = []; // promises
    ...
    do {
      ...
      for (const entry of Contents) {
        if (entry.Key.length < s3Path.length) {
          ...
          // existingFolders.add(targetPath); // This would prevent children from being added.
          continue;
        }
        ...
        if (folderPath && !relativePath.endsWith('/')) { // this folder contains at least one document
          ...
          if (Array.from(existingFolders).some(existingFolderPath => folderPath.startsWith(existingFolderPath))) {
    ...
    await Promise.allSettled(headQueue);
    ...
    for (const [folderPath, folder] of shallowToDeep) {
      try {
        if (!existingFolders.has(folderPath) && Object.keys(folder).length > 0) {
          ...
          puts.push(putPrms);
          logNotes.add(`cached ${s3Path} ${folderETag}`);
        }
      } catch (err) {
    ...
    function processEntry (entry, folderPath, itemName) {
      ...
      if (itemName.endsWith('/')) {
        ...
        existingFolders.add(posix.join(folderPath, itemName));
      } else if (TYPE_SUFFIX_PATT.test(entry.Key)) {
        // set Content-Type on item
      } else {
        folder[itemName] = {
        ...
        if (!folder[itemName].ContentType) {
          const headPrms = s3send(new HeadObjectCommand({
            Bucket: bucketName,
            Key: s3Path + folderPath + itemName
          })).then(headResult => {
            if (headResult.DeleteMarker) {
              folder[itemName] = undefined;
            } else if (folder[itemName]) { // not a false entry
              folder[itemName] = {
  ...
  router.put('/:username/*',
    ...
    async function (req, res, next) {
      try {
        ...
        const { Contents } =
            await s3send(new ListObjectsV2Command({ Bucket: bucketName, Prefix: s3Path }));
        if (Contents?.some(entry => entry.Key.startsWith(s3Path + '/'))) {
          res.logNotes.add('blocked attempt to overwrite folder');
          res.status(409).type('text/plain').send('is actually folder: ' + s3Path.slice(BLOB_PREFIX.length));
          return;
        }
        const targetEntry = Contents?.find(entry => entry.Key === s3Path);
        const currentETag = targetEntry?.ETag || null;
        ...
        const putETag = await putBlob(bucketName, s3Path, contentType, contentLength, req, currentETag);
        res.logNotes.add(putETag);

        await invalidateAncestors(bucketName, s3Path, res.logNotes);

        res.status(currentETag ? 200 : 201).set('ETag', putETag).end();
        reduceS3PauseBecauseOfSuccess();
      } catch (err) {
        if (err.name === 'EndResponseError') {
          res.logNotes.add(err.message);
          res.logLevel = err.logLevel;
          return res.status(err.statusCode).type('text/plain').send(err.message);
        } else if (['PreconditionFailed', '412'].includes(err.name) || err.code === 'PreconditionFailed') {
          errToMessages(err, res.logNotes);
      ...
      try {
        ...
        try {
          if (['NoSuchBucket', 'AccessDenied'].includes(err.Code) ||
            ...
            if (Contents?.some(entry => entry.Key.startsWith(s3Path + '/'))) {
              res.logNotes.add('blocked attempt to overwrite folder');
              return res.status(409).type('text/plain').send('can\'t delete folder directly: ' + s3Path.slice(BLOB_PREFIX.length));
            } else {
              errToMessages(err, res.logNotes);
  ...
  async function checkAncestorsAreFolders (bucketName, s3Path) {
    ...
    do {
      try {
        const { ContentType } = await s3send(new HeadObjectCommand({ Bucket: bucketName, Key: ancestorPath }));
        if (ContentType !== FOLDER_MIME_TYPE) {
  ...
  async function invalidateAncestors (bucketName, s3Path, logNotes) {
    const deletes = [];
    const segments = s3Path.split('/');
    for (let i = segments.length - 1; i >= 1; --i) {
      const ancestorPath = segments.slice(0, i).join('/') + '/';
      deletes.push(s3send(new DeleteObjectCommand({ Bucket: bucketName, Key: ancestorPath })));
    }
    const outcomes = await Promise.allSettled(deletes);
    for (const outcome of outcomes) {
      if (outcome.status === 'rejected') {
        errToMessages(outcome.reason, logNotes);
      }
    }
  }
  ...
  function rejectIfBusy (req, res, next) {
    try {
      const numWaitingS3Requests = Object.values(s3Agent.requests).reduce((sum, arr) => sum + arr.length, 0);
      if (numWaitingS3Requests > MAX_WAITING_S3_REQUESTS) {
        res.logNotes.add(`${numWaitingS3Requests} waiting requests; retry-after ${BUSY_RETRY_AFTER}s`);
        res.set({ 'Retry-After': BUSY_RETRY_AFTER });
        return res.status(429).end();
      }

      if (numS3Pauses > 0) {
        getLogger().debug(`${Date.now() % 100_000} rejectIfBusy: numS3Pauses: ${numS3Pauses}   s3PauseMs: ${s3PauseMs / 1000}s`);
