...
module.exports = function ({ endPoint = 'play.min.io', accessKey = 'Q3AM3UQ867SPQQA43P2F', secretKey = 'zuf+tfteSlswRu7BJ86wekitnifILbZam1KYY3TG', region = 'us-east-1', userNameSuffix = '' }) {
          ...
          if (['NoSuchBucket', 'AccessDenied'].includes(err.Code) ||
            ...
            if (isFolderRequest) {
              ...
              if (Contents?.length > 0) {
                ...
                return res.status(409).type('text/plain').send('is actually folder: ' + s3Path.slice(BLOB_PREFIX.length + 1));
              } else {
                errToMessages(err, res.logNotes);
                return res.status(404).end(); // Not Found
              }
  ...
  async function listFolder (username, targetPath, logNotes) {
    ...
    do {
      ...
      if (Contents.find(entry => entry.Key === calcS3Path(BLOB_PREFIX, targetPath).slice(0, -1)) &&
          Contents.filter(entry => entry.Key.startsWith(s3Path)).length === 0) {
        throw new EndResponseError('is document, not folder', undefined, 409);
      }

      for (const entry of Contents) {
        if (entry.Key.length < s3Path.length) {
          // There's a file at this targetPath TODO: check before for loop for edge cases
          continue;
        } else if (entry.Key === targetPath) {
        ...
        if (folderPath) {
          ...
          if (grandparent?.[basename(folderPath) + '/']) {
            // We already have an ETag for this entry's folder.
            // Ignores anything within the folder.
            continue;
          }
        }
    ...
    async function (req, res, next) {
      try {
        ...
        if (Contents?.some(entry => entry.Key.startsWith(s3Path + '/'))) {
          ...
          res.status(409).type('text/plain').send('is actually folder: ' + s3Path.slice(BLOB_PREFIX.length + 1));
          return;
        }
      ...
      try {
        ...
        try {
          if (['NoSuchBucket', 'AccessDenied'].includes(err.Code) ||
            ...
            if (Contents?.some(entry => entry.Key.startsWith(s3Path + '/'))) {
              res.logNotes.add('blocked attempt to overwrite folder');
              return res.status(409).type('text/plain').send('can\'t delete folder directly: ' + s3Path.slice(BLOB_PREFIX.length + 1));
            } else {
              errToMessages(err, res.logNotes);
