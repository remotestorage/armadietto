  ...
  router.put('/:username/*',
    ...
    async function (req, res, next) {
      try {
        ...
        for (const entry of Contents || []) {
          let match;
          if ((match = typeCacheRegEx.exec(entry.Key))) {
            if (calcContentType(match[1]) !== contentType) {
              await s3send(new DeleteObjectCommand({ Bucket: bucketName, Key: entry.Key }));
            }
          }
        }
      ...
      try {
        ...
        if (keys?.length > 0) {
          ...
          await waitUntilObjectNotExists(
            { client: s3client, maxWaitTime: 9 }, // 1 sec before next sync
            { Bucket: bucketName, Key: s3Path }
          );
        }
