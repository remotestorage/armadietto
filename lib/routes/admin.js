...
const assembleContactURL = require('../util/assembleContactURL');
...
module.exports = async function (hostIdentity, jwtSecret, accountMgr, storeRouter) {
  ...
  router.get('/acceptInvite',
    ...
    loadAdmin,
    async (req, res) => {
      res.set('Cache-Control', 'no-store'); // set this because this request can't be a POST
      try {
        res.logNotes.add(`admin ${req.session.user?.username || '«new»'} redeeming ${req.query?.token} "${req.session.user?.contactURL}"`);
        ...
        res.status(401).render('login/error.html', {
          title: 'Your invitation is not valid',
          host: getHost(req),
          privileges: req.session.privileges || {},
          accountPrivileges: req.session.user?.privileges || {},
          subtitle: '(or there was an error)',
          message: 'Request another invitation using the link above.'
        });
      }
    }
  );
  ...
  router.post('/getRegistrationOptions',
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (!admin) { throw new Error('Request another invite'); }

        const newUsername = req.body.username?.trim();
        if (newUsername) { admin.username = newUsername; }
        if (!admin.username) { throw new ParameterError('username is required'); }
        await storeRouter.upsertAdminBlob(path.join(CONTACT_URL_DIR, encodeURIComponent(admin.contactURL) + '.yaml'), 'application/yaml', admin.username);

        if (req.session.isUserSynthetic) {
          admin = await accountMgr.createUser(admin, res.logNotes);
          delete req.session.isUserSynthetic;
          res.logNotes.add(`created user ${admin.username} "${admin.contactURL}"`);
          isCreate = true;
        }
        req.session.username = admin.username;

        const excludeCredentials = Object.values(admin?.credentials || {}).map(credential => ({
        ...
        const options = await generateRegistrationOptions({
          rpName,
          rpID,
          userID: Buffer.from(admin.username, 'utf8').toString('base64url'),
  ...
  router.post('/verifyRegistration',
    ...
    async (req, res) => {
      try {
        res.set('Cache-Control', 'no-store');
        await storeRouter.deleteAdminBlob(path.join(ADMIN_INVITE_DIR_NAME, req.query?.token + '.yaml'));
        ...
        const credential = Object.assign({}, registrationInfo,
          { transports: req.body?.response?.transports, name: nameFromUseragent(req.useragent), createdAt: new Date() });
        req.session.user.credentials[Buffer.from(registrationInfo.credentialID.buffer).toString('base64url')] = credential;
        await accountMgr.updateUser(req.session.user.username, req.session.user, res.logNotes);
  ...
  async function loadAdmin (req, res, next) {
    try {
      removeUserDataFromSession(req.session);
      const inviteFile = await storeRouter.readAdminBlob(path.join(ADMIN_INVITE_DIR_NAME, req.query?.token + '.yaml'));
      const adminInvite = YAML.parse(inviteFile);
      const contactURL = new URL(adminInvite.contactURL); // validates & normalizes
      if (!adminInvite.expiresAt || !(Date.now() <= new Date(adminInvite.expiresAt))) {
        await storeRouter.deleteAdminBlob(path.join(ADMIN_INVITE_DIR_NAME, req.query?.token + '.yaml'));
        throw new Error('invite expired');
      }

      let username = adminInvite.username?.trim();
      if (username) {
      ...
      if (username) {
        try {
          admin = await accountMgr.getUser(username, res.logNotes);
          admin.privileges ||= adminInvite.privileges; // do we really need this backstop?
      ...
      if (!admin) {
        delete adminInvite.expiresAt;
        admin = { credentials: {}, ...adminInvite, ...(username && { username }) };
  ...
  router.bootstrap = async function bootstrap () {
    ...
    try {
      ...
      const filePath = path.join(ADMIN_INVITE_DIR_NAME, `${encodeURIComponent(contactURL)}.uri`);
      try {
        await storeRouter.metadataAdminBlob(filePath);
        getLogger().info(`invite already sent to ${contactURL}`);
        return;
      } catch (err) {
        if (!['NoSuchBlobError', 'NoSuchKey'].includes(err.name)) {
          throw err;
  ...
  router.generateInviteURL = async function (contactStr, username, privileges = { STORE: true }) {
    ...
    await storeRouter.upsertAdminBlob(path.join(ADMIN_INVITE_DIR_NAME, token + '.yaml'), 'application/yaml',
  ...
  router.inviteAdmin = async function inviteAdmin (contactUrl, _inviteURL, _msg) {
    await new Promise(resolve => setTimeout(resolve, 1000));

    switch (contactUrl.protocol) {
      case 'sgnl:':
        throw new Error('inviting user via Signal Private Messenger not yet implemented');
      case 'threema:':
        throw new Error('inviting user via Threema not yet implemented');
      case 'mailto:':
        throw new Error('inviting user via email address not yet implemented');
      case 'sms:':
      case 'mms:':
        throw new Error('inviting user via text messaging not yet implemented');
      case 'facetime:':
        throw new Error('inviting user via FaceTime not yet implemented');
      case 'skype:':
        throw new Error('inviting user via Skype not yet implemented');
      case 'ssh:':
        throw new Error('inviting user via SSH address not yet implemented');
      default:
        throw new Error(`Get real. How could you send a token via “${contactUrl}”?`);
    }
  };
      ...
      try {
        ...
        if (privilegeGrant.ADMIN) {
          ...
          text = req.body.username
            ? `${req.body.username}, to create a passkey for ${hostIdentity} on a new device or browser, copy and paste this URL into the browser on that device:`
