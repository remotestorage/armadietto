  ...
  router.get('/:username',
    ...
    async function (req, res) {
      try {
        ...
        req.session.oauthChallenge = options.challenge; // Saves the challenge in the user session
  ...
  router.post('/',
    ...
    async function (req, res) {
      try {
        ...
        const authenticationInfo = await verifyCredential(user, req.session.oauthChallenge, origin, rpID, presentedCredential);
        delete req.session.oauthChallenge; // Kills the challenge for this session.
        ...
        const token = jwt.sign(
          { scopes },
          jwtSecret,
          { algorithm: 'HS512', issuer: hostIdentity, audience: redirectOrigin, subject: req.data.username, expiresIn: '7d' }
        );
        res.logNotes.add(`created JWT for ${req.data.username} on ${redirectOrigin} w/ scope ${scopes}`);
        const args = {
          access_token: token,
          token_type: 'bearer',
        ...
        res.status(401).render('auth-passkey.html', {
          ...
          error: error.message,
          client_host: req.body.redirect_uri ? new URL(req.body.redirect_uri).host : '[missing origin]',
          client_id: req.body.client_id,
          redirect_uri: req.body.redirect_uri,
          response_type: req.body.response_type,
          scope: req.body.scope || '',
          state: req.body.state || '',
          permissions: parseScope(req.body.scope || ''),
