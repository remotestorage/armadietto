...
const { POINTS_UNAUTH_REQUEST, rateLimiterPenalty, rateLimiterBlock } = require('../middleware/rateLimiterMiddleware');
    ...
    async function (req, res) {
      try {
        ...
        res.status(401).render('login/error.html', {
          ...
          accountPrivileges: req.session.user?.privileges || {},
          subtitle: '',
          message: 'If you need to create a passkey for this device or browser, log in from your old device and invite yourself to create a new passkey, or contact an administrator.'
        });
      }
    });
    ...
    async function (req, res) {
      try {
        if (!req.session.oauthParams) {
          return errorPage(req, res, 401,
            { title: 'Authorization Failure', message: 'Go back to the app then try again — your session expired' },
            'no oauthParams — session expired?'
          );
        }
