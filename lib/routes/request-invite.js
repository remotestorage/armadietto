  ...
  router.post('/',
    async function (req, res) {
      ...
      try {
        ...
        const contactURL = assembleContactURL(req.body.protocol, req.body.address);

        await storeRouter.upsertAdminBlob(path.join(INVITE_REQUEST_DIR, encodeURIComponent(contactURL.href) + '.yaml'), 'application/yaml', contactURL.href);
        ...
        res.status(201).render('login/request-invite-success.html', {
          ...
          privileges: {},
          accountPrivileges: {},
          params: { contactURL: contactURL.href }
        });
      } catch (err) {
