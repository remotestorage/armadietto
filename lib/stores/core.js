...
const core = {
  ...
  indexed (list) {
    return list.map((v, i) => ({index: i, value: v}))
  },
  ...
  hashPassword (password, config, callback) {
    config = config || {
    ...
    crypto.pbkdf2(password, config.salt, parseInt(config.work, 10),
      parseInt(config.keylen, 10), 'sha512',
      (error, key) => {
        config.key = Buffer.from(key, 'binary').toString('base64')
        callback(error, config)
      })
  },

  parents (path, includeSelf) {
    let query = core.parsePath(path)
    let parents = []
