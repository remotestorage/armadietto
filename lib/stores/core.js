...
const core = {
  ...
  validateUser (params,
    validNamePatt,
    validCharset = 'letters, numbers, dots, dashes and underscores') {
    const errors = [];
    ...
    if (!core.isValidUsername(username, validNamePatt)) { errors.push(new Error(`Usernames may only contain ${validCharset}`)); }
  ...
  isValidUsername (username, validNamePatt = this.VALID_NAME) {
    if ((!username) || (username === '..')) return false;
    return validNamePatt.test(username);
  },

  versionMatch (versions, modified) {
