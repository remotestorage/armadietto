...
class CouchDB {
  ...
  async createUser (params) {
    ...
    try {
      ...
      do {
        ...
        try {
          await perUserDB.insert(RSDesignDoc, '_design/remoteStorage');
          ...
          if (err.statusCode !== 404 || Date.now() - startTime > 60_000) {
            throw err;
          } else {
   ...
   * @param {string} clientAddress the IP address of the user's machine
   * @param {string} host the Host or X-Forwarded-Host request header
   * @returns {Promise<{}|null>} permissions object (possibly empty), or null if token is not valid
   */
  async permissions (username, token, clientAddress, host) {
    const requestDefaults = { jar: true, headers: {} };
    if (clientAddress) {
      requestDefaults.headers['X-Forwarded-For'] = clientAddress;
    }
    if (host) {
      requestDefaults.headers.Host = host;
    }
    const perUserDB = nanoConnect({
