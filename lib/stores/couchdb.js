...
function (doc) {
  ...
  for (let i=1; i<doc.pathArr.length; ++i) {
    emit(doc.pathArr.slice(0,i), {etagN: 0, etagHash: [], dirCount:1, contentType: [], contentLength:0, lastModified: 0});
  }
  ...
  emit(doc.pathArr, {etagN, etagHash, dirCount:0, contentType, contentLength, lastModified: Date.parse(doc.lastModified)});
}`,
      reduce: '_sum'
    }
  },
