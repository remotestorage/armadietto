 ...
 * CouchDB users not created by Armadietto would need to have _security,
 * and design documents created manually.
 *
 * If users don't need to be created, no admin credentials need be supplied.
...
class CouchDB {
  ...
  async createUser (params) {
    ...
    try {
      ...
      do {
        ...
        try {
          /* const response1 = */ await perUserDB.insert({
  ...
  async authenticate (params) {
    ...
    // console.log("perUserDB.session:", await perUserDB.session())

    this._perUserDB[params.username] = perUserDB;
    return perUserDB;
  }

  async readAuth (username, perUserDB) {
    const authorization = await perUserDB?.get('remoteStorageAuthorization');
    // console.log(`read “${username}” authorization from ${perUserDB?.config?.db}:`, authorization, nano?.config?.cookies?.[0]);
    return authorization;
  }

  async writeAuth (username, auth, perUserDB) {
    try {
      /* const response = */ await perUserDB.insert(auth, 'remoteStorageAuthorization', auth._rev);
      // console.log(`wrote “${username}” auth to ${perUserDB?.config?.db}:`, auth, response?.rev);
    } catch (err) {
      const msg = `failed to write “${username}” auth to ${perUserDB?.config?.db}: ${err.statusCode} ${err.description} ${err.url}`;
      getLogger().error(msg);
      throw err;
    }
  }
  ...
  async authorize (clientId, username, password, permissions) {
    ...
    let auth;
    try {
      auth = await this.readAuth(username, perUserDB);
    } catch (err) {
    ...
    getLogger()?.notice(`“${username}” authorized new client with ${JSON.stringify(session)}`);

    await this.writeAuth(username, auth, perUserDB);

    return clientPermissionsToken;
  }

  /**
   * Retrieves permissions given to client with this token.
   * @param {string} username
   * @param {string} token returned from authorize()
  ...
  async permissions (username, token) {
    ...
    try {
      auth = await this.readAuth(username, perUserDB);
      // console.log('permissions:', auth?.sessions?.[token]?.clientId, auth?.sessions?.[token]?.permissions);
