  ...
  async revokeAccess (username, token) {
    ...
    try {
      ...
      if (auth?.sessions?.[token]) {
        // console.log('removing authorization:', auth?.sessions?.[token]?.clientId, auth?.sessions?.[token]?.permissions);
        delete auth?.sessions?.[token];
        await this.writeAuth(username, auth, perUserDB);
      }
    } catch (err) {
      if ([401, 403, 404].includes(err.statusCode)) {
        getLogger()?.info(`User “${username}” lacks permission to revoke token in “${perUserDB?.config?.db}” ` + JSON.stringify(auth?.sessions?.[token]));
        return undefined;
      } else {
        throw err;
      }
