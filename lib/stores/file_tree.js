...
class FileTree {
  ...
  async createUser (params) {
    ...
    const hash = await core.hashPassword(params.password);
  ...
  async permissions (username, token) {
    const user = this.readAuth(username);
  ...
  async get (username, path, version, callback) {
    ...
    if (isdir) {
      ...
      return {children: listing, modified: mtime};//, this._versionMatch(version, mtime));
    } else {
  ...
  getCurrentState (fullPath, version) {
    const statInfo = stat(fullPath);
