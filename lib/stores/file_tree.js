...
class FileTree {
  ...
  _metaPath (username, pathname, isdir) {
  ...
  dataPath (username, pathname) {
    return path.join(this._resolvePath(username, 'storage/' + pathname));
  }
  ...
  async get (username, pathname, version, head = false) {
  ...
  async put (username, pathname, type, value, version) {
    ...
    try {
      ...
      metaData.ETag = modified.toString();
      await writeFile(metaPath, JSON.stringify(metaData, true, 2));
      // update all the hierarchy
      await this._updateParents(username, pathname, modified.toString());
      await this._unlock(username);
  ...
  async _updateParents (username, pathname, version) {
    ...
    return Promise.all(parents.map(async p => {
      const metaPath = this._metaPath(username, p);
      const metaData = await this.readMeta(username, p, false);
      metaData.ETag = version;
      return writeFile(metaPath, JSON.stringify(metaData, true, 2));
    }));
  }
  ...
  async writeFile (fullPath, content) {
    ...
    try {
      ...
      currentDirname = path.dirname(fullPath);
      prevDirname = path.dirname(currentDirname);
      prevMetadata = (await this.readJson(path.join(prevDirname, '.~meta'))) || {items: {}};
      await mkdirp(path.dirname(fullPath));
    }

    await writeFile(fullPath, content);
    statInfo = statInfo || await stat(fullPath);
    if (prevDirname) {
      prevMetadata.items[path.basename(currentDirname)] = { ETag: statInfo && statInfo.mtime.getTime() };
      await writeFile(path.join(prevDirname, '.~meta'), JSON.stringify(prevMetadata, true, 2));
    }
