...
class FileTree {
  ...
  async get (username, pathname, versions, head = false) {
    ...
    // resource exists?
    let ret;
    if (!metadata.ETag) ret = {item: isdir ? { items: {} } : null};
    ...
    try {
      ...
      const tasks = paths.map(({currentPath, upperBasename}) => async () => {
        ...
        if (Object.keys(upperMeta.items).length === 0) {
          ...
          await unlink(this._metaPath(username, currentPath, true));
          await rmdir(this.dataPath(username, currentPath));
          await writeFile(this._metaPath(username, currentPath), JSON.stringify(currentMeta, true, 2));
        }
        upperMeta = currentMeta;
      });

      // resolve these promises sequentially (you need to delete upper dir first)
      await tasks.reduce((promise, func) => {
        promise.then(result => func().then(Array.prototype.concat.bind(result)));
      }, Promise.resolve([]));

      this._updateParents(username, pathname, metadata.ETag);
    } catch (e) {
      await this._unlock(username);
      return {deleted: false};
    }
    await this._unlock(username);
    return {modified: metadata.ETag, deleted: true};
  }
