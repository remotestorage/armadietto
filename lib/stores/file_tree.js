...
class FileTree {
  ...
  async delete (username, pathname, version) {
    ...
    if (!exists || conflict) {
      console.error(`DENTRO DELETE di ${pathname} non esiste!!`);
      await this._unlock(username);
    ...
    await this._unlock(username);
    return {exists, modified, deleted: true};
  }
  ...
  async _delete (username, pathname, version) {
    ...
    console.error(`DOPO getCurrentState di ${pathname} con ${version} => ${current}`);
