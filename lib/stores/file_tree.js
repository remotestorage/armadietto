...
class FileTree {
          ...
          this.childPaths(username, path, entries => {
            ...
            Promise.all(entries.map((entry) => {
              return new Promise((resolve, reject) => {
                this._getListing(username, path, entry, (error, listing) => {
  ...
  async put (username, pathname, type, value, version, callback) {
    const query = core.parsePath(pathname)
    try {
      [a, b] = await new Promise((res, rej) => { resolve([a, b])});

      [c, d] = await new Promise((res, rej) => { resolve([a, b])})
    } catch (e) {

    }
    this._lock(username,async release => {
      ...
      this.getCurrentState(dataPath, version, async (error, current) => {
        if (error || !current) {
          release()
        ...
        try {
          [exists, modified] = await new Promise((resolve, reject) => {
            this.writeBlob(metaPath, JSON.stringify({length: value.length, type: type}, true, 2), (error, exists, modified) => {
              if (error) reject(error)
          ...
          ;[exists, modified] = await new Promise((resolve, reject) => {
            this.writeBlob(dataPath, value, (error, exists, modified) => {
              if (error) reject(error)
              else resolve([exists, modified])
            })
          })
          ...
          // touch all entries in parallel
            Promise.all(core.indexed(query).map((entry) => {
              return new Promise((resolve, reject) => {
                const i = entry.index
                this.touch(this.dirname(username, query.slice(0, i + 1).join('')), modified, (error) => {
                  if (error) reject(error)
                  else resolve()
                })
              })
