...
class FileTree {
  ...
  async get (username, pathname, version, head = false) {
    ...
    if (metaData && metaData.items && metaData.items[filename] &&
      this._versionMatch(version, metaData.items[filename].ETag)) {
  ...
  /**
   * Update all parents ETag
   */
  async _updateParents (username, pathname, version) {
    const parents = core.parents(pathname, false);
    for (let i = 1; i < parents.length; i++) {
      const metaPath = this._metaPath(username, parents[i], true);
      const metaData = await this.readMeta(username, parents[i], false);
      metaData.ETag = version;
      metaData.items[path.basename(parents[i - 1]) + '/'] = { ETag: version };
      await writeFile(metaPath, JSON.stringify(metaData, true, 2));
    }
  }
  ...
  async getCurrentState (fullPath, version) {
    ...
    return { current: modified === version, modified: modified.toString(), isDir };
  }
}
