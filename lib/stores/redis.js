...
class RedisStore {
    ...
    if (isdir) {
      ...
      return Promise.all(children.sort().map((child) => {
        return client.hget(key + child, 'modified')
          .then(modified => ({name: child, ETag: parseInt(modified, 10)}));
      })).then((listing) => {
  ...
  async put (username, pathname, type, value, version) {
    ...
    core.indexed(query).forEach((q, i) => {
      ...
      multi.hset(key, 'ETag', modified);
      multi.sadd(key + ':children', query[i + 1] || filename);
    });

    multi.hmset(dataKey, {'Content-Length': value.length, 'Content-Type': type, ETag: modified, value: value.toString('base64')});
  ...
  async _lock (username) {
    ...
    const retry = () => {
      return new Promise((resolve, reject) => {
        setTimeout(async () => { await this._lock(username); resolve(); }, 100);
      });
    };

    if (await client.setnx(lockKey, expiry) === 1) return;

    const timeout = await client.get(lockKey);
