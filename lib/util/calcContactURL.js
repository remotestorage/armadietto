...
module.exports = function calcContactURL (contactStr) {
  ...
  if (!['sgnl:', 'threema:', 'mailto:', 'sms:', 'mms:', 'skype:', 'facetime:', 'xmpp:', 'whatsapp:', 'tg:', 'ymsgr:', 'ssh:'].includes(contactURL.protocol)) {
    throw new ParameterError(`Not possible to contact user via “${contactURL.href}”`);
  }
  if (!['sgnl:', 'threema:', 'skype:', 'whatsapp:', 'tg:', 'ymsgr:'].includes(contactURL.protocol)) {
    contactURL.search = '';
  }
  contactURL.hash = '';
